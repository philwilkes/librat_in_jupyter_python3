...
def camera(fname,
    ...
    if os.path.isfile(fname) and not overwrite:
        R = input('this camera exists, overwrite? (y/n)').lower()
        if R != 'y':
            sys.exit()
    ...
    for K, V in list(camera_options.items()):
        camera[K] = V   
    ...
    for K, V in list(geometry_options.items()):
        geometry[K] = V
    ...
    for K, V in list(sampling_options.items()):
        sampling[K] = V
     
    ### setting results
    if 'oname' in list(results_options.keys()):
    ...
    camera_settings = OrderedDict(list(camera.items()) + 
                                  list(geometry.items()) +
                                  list(sampling.items()) +
                                  list(results.items()))
...
def write_camera(fname, camera_settings, comment, verbose):
    ...
    with open(fname, 'w') as cam:
        ...
        for K, V in list(camera_settings.items()):
